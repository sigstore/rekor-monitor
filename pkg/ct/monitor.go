...
func MatchedIndices(logEntries []ct.LogEntry, mvs identity.MonitoredValues, trustedCAs []string) ([]identity.LogEntry, []identity.FailedLogEntry, error) {
	matchedEntries := []identity.LogEntry{}
	...
	for _, entry := range logEntries {
		if entry.X509Cert != nil {
			...
			if err == nil {
				if err = identity.ValidateCertificateChain([]*x509.Certificate{cert}, trustedCAs); err != nil {
		...
		if entry.Precert != nil {
			...
			if err == nil {
				if err = identity.ValidatePreCertificateChain([]*google_x509.Certificate{cert}, trustedCAs); err != nil {
...
func IdentitySearch(ctx context.Context, client *ctclient.LogClient, startIndex int64, endIndex int64, monitoredValues identity.MonitoredValues, outputIdentitiesFile string, idMetadataFile *string, trustedCAs []string) ([]identity.MonitoredIdentity, []identity.FailedLogEntry, error) {
